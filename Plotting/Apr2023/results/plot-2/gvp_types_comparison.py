...
name_list = ["GVP same sample", "GVP split sample", "GVP split sample absolute value"]
color_list = ["red", "blue", "green", "black", "purple", "orange"]
jfirst_list = [False, False, False]
...
def read_files(file_names, all_energies, all_variances, all_uncert_of_varriances, all_stdevs, all_stderrs, all_target_functions, all_target_fn_stderrs, all_grad_norms, all_lderivs, all_param_update_sizes, all_largest_param_updates, all_qmc_timings, all_total_timings, max_files=None):
    current_energies = []
    current_variances = []
...
def multiplot_avgN(multi_all_files, multi_all_energies, num_rows, num_cols, subplot_titles, N1=10, N2=10):
    ...
    for j, all_files in enumerate(multi_all_files):
        if j == 0:
            plot_avgN(all_files, multi_all_energies[j], N1=N1, N2=N2, ax=flat_ax_list[j], title=subplot_titles[j], need_names=True)
        else:
            plot_avgN(all_files, multi_all_energies[j], N1=N1, N2=N2, ax=flat_ax_list[j], title=subplot_titles[j], need_names=False)
        # Only label the "outer" y axes
        if j % num_cols != 0:
...
def plot_avgN(all_files, all_energies, N1=10, N2=10, ax=None, title=None, need_names=True):
    need_legend = False
    if ax == None:
        ax = plt.gca()
        need_legend = True
    ...
    avg_N_list = []
    err_N_list = []
    for i, energies_list in enumerate(all_energies):
        avg_N_list = []
        err_N_list = []
        for j, energies in enumerate(energies_list):
            file = all_files[i][j]
            if j%2 == 0:
            ...
            if j == 0 and need_names == True:
                print(f"adding label {name_list[i]}")
        ...
        every_second_x_list = [(j+1)/2 for j in range(len(energies_list)) if j%2 == 1]
...
def main():
    ...
    plot_energies(shrunk_h2o1b1_energies, errbars=shrunk_h2o1b1_err, avgN=1, fig_title='H2O 1B1 State')
    plot_target_functions(shrunk_h2o1b1_targetfn, errbars=shrunk_h2o1b1_target_err, avgN=1, fig_title='H2O 1B1 State')
    plt.show()
    ...
    ##########
    # Read the files to get data
    formgs_files = [gvp_formgs_filenames, gvp_formgs_split_filenames, gvp_formgs_splitabs_filenames]
    ...
    # Plot the data
    # plot_avgN(formgs_files, formgs_energies, N1=100, N2=10)
    # plot_energies(shrunk_formgs_energies, errbars=shrunk_formgs_err, avgN=1, fig_title='Formaldehyde Ground State')
    ...
    #################
    # Read the files to get data
    formn2pistar_files = [gvp_formn2pistar_filenames, gvp_formn2pistar_split_filenames, gvp_formn2pistar_splitabs_filenames]
    ...
    # Plot the data
    plot_avgN(formn2pistar_files, formn2p_energies, N1=100, N2=10)
    plot_energies(shrunk_formn2p_energies, errbars=shrunk_formn2p_err, avgN=1, fig_title='Formaldehyde N to Pi* State')
    plot_target_functions(shrunk_formn2p_targetfn, errbars=shrunk_formn2p_target_err, avgN=1, fig_title='Formaldehyde N to Pi* State')
    plt.show()
    ...
    #################
    # Read the files to get data
    formp2pistar_files = [gvp_formpi2pistar_filenames, gvp_formpi2pistar_split_filenames, gvp_formpi2pistar_splitabs_filenames]
    ...
    # Plot the data
    plot_avgN(formp2pistar_files, formp2p_energies, N1=100, N2=10, title="Formaldehyde Pi to Pi* State")
    plot_energies(shrunk_formp2p_energies, errbars=shrunk_formp2p_err, avgN=1, fig_title='Formaldehyde Pi to Pi* State')
    plot_target_functions(shrunk_formp2p_targetfn, errbars=shrunk_formp2p_target_err, avgN=1, fig_title='Formaldehyde Pi to Pi* State')
    plt.show()
    ...
    # Energies
    multiplot_final_lines(multi_all_energies, 2, 3, avgN=1, multi_errbars=multi_all_energy_errs, fig_title="Energies", subplot_titles=['Formaldehyde Ground State', 'Formaldehyde N to Pi* State', 'Formaldehyde Pi to Pi* State', 'H2O Ground State', 'H2O 1B1 State'], ylabel="Energy (Ha)", xlabel="Macroiterations")
    # plt.show()
